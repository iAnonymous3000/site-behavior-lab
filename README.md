...
**CI-gated deployment.** Production (Cloudflare Pages and the scanner's Workers Builds) tracks the **`production`** branch, never `main`. The `.github/workflows/promote-production.yml` workflow is the only thing that moves it: after a `main` CI run succeeds, it fast-forwards `production` to the exact SHA that run tested. It never force-pushes, skips out-of-order completions (an older run finishing after a newer promotion), and hard-fails on a tested SHA no longer reachable from `main`. The repo-writing workflows (scan, scan-featured, update-brave-lists) dispatch CI after their `GITHUB_TOKEN` pushes, so corpus and filter-list commits are tested and promoted like any other change. Operator setup, once: point the Cloudflare Pages project's production branch and the scanner's Workers Builds branch at `production`, and disable non-production builds on both.
