...
The scan workflow prunes committed static reports before updating the manifest. By default it keeps reports for 7 days up to a hard ceiling of 1,000 committed reports, except that each site's newest two reports of each kind (Shields, consent, GPC, single) are exempt from age pruning (tune with `SITE_BEHAVIOR_LAB_STATIC_REPORT_KEEP_PER_SITE`, 0 restores pure age pruning): a site keeps a current and a previous generation for the directory's "changed since last scan" view, and a site that stops being re-scanned never silently drops out of the corpus. Override static retention with `SITE_BEHAVIOR_LAB_STATIC_REPORT_MAX_AGE_DAYS` and `SITE_BEHAVIOR_LAB_STATIC_REPORT_MAX_COUNT`, or use the existing `SITE_BEHAVIOR_LAB_REPORT_MAX_AGE_DAYS` and `SITE_BEHAVIOR_LAB_REPORT_MAX_COUNT` variables as shared fallbacks.

The featured-scan workflow also runs on a weekly schedule (Mondays 05:23 UTC) with its default inputs (Shields comparisons over the featured catalog), so the corpus refreshes itself. On `/directory/`, each site's newest report shows what changed since the previous scan of the same kind ("+12 third-party, -3 tracker requests"); kinds never mix, because a consent comparison's lead run (an accept-click visit) is not comparable to a Shields comparison's lead run (pre-consent). These deltas are observed differences between two automated visits, which can also reflect ad rotation, experiments, caching, or bot detection, and the UI says so.

## Researcher Export

`/corpus.json` and `/corpus.csv` export the committed corpus with one row per published report: domain, category, absolute report and JSON URLs, scan date, report kind (Shields / consent / GPC / single), device, GPC and consent mode of the lead run, the lead run's HTTP status (400 or higher marks an error or block page, not the site's normal behavior; exclude those rows from aggregate statistics, as this project's own percentiles and category medians do), plain-language headline, the third-party / catalogued-service / cookie counts, the observed blocking reduction (the `shields_third_party_reduction` column: fewer third-party requests observed in the Brave-list blocking visit of an eligible paired comparison, a simulation with Brave's engine and default lists in the scanner's browser rather than a live Brave visit, blank otherwise; never a count of individually blocked requests), the since-last-scan deltas with the previous report's id, and the schema columns (`schema_version`/`schema_revision` for the wire generation, `schema_origin` marking legacy-derived v1 rows, and `limited` marking rows whose revision supports only descriptive claims; every current row is v1, legacy-derived, and limited). Both are generated at build time from the same loader as `/directory/`, so the three surfaces cannot disagree; the JSON payload embeds the framing note and the CSV stays header-plus-rows for parsers. This is a measured corpus of curated sites (popular, mostly commercial, plus a diversity seed list), not a random sample of the web: treat cross-site statistics as describing this corpus only, and cite per-report evidence via the linked report pages. Licensed AGPL-3.0-or-later with the repository.
...
| Brave PageGraph adapter | yes | no | no | no | no | source artifact | provided or bundled curated catalog | caller-managed |
