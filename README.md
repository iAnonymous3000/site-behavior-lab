...
> **Deployment status.** The public site at [https://sitebehavior.org](https://sitebehavior.org) is the static **Cloudflare Pages** export, with the Cloudflare **Browser Run Worker** as its live **GPC / trackers** scanner. The full **Node/Playwright scanner — including live Brave Shields (the tried-vs-blocked diff) — is deployed on Cloudflare Containers** with R2-backed report storage, running **operator-gated** (see [docs/deploy-cloudflare-containers.md](docs/deploy-cloudflare-containers.md)). Point the Pages site's `NEXT_PUBLIC_SITE_BEHAVIOR_LAB_SCAN_API_BASE` at it to surface live Shields in the public UI — see [docs/go-live-public-scanner.md](docs/go-live-public-scanner.md) for the safe (Turnstile + rate-limit + WAF) go-live sequence. Shields evidence is also generated operator/CI-side and published into the static evidence corpus. See [docs/deployment-topology.md](docs/deployment-topology.md) for the decision record.
