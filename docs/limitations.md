...
holds each dedicated worker paused before its first statement, and installs the
same fingerprint observer inside it during that pause; only the GPC arm also
installs the signal, first, so for verified workers a GPC comparison differs
between arms only in the signal itself; an unverified worker is the remaining
one-arm asymmetry and is disclosed. The pause itself is a change to the visit in
every arm, baseline included: in local measurements of a prototype of this
install on one machine, it delayed each dedicated worker's start by 3 to 6 ms,
and a burst of eight workers by 7 to 8 ms.
...
The fingerprinting section is an observation layer, not a definitive accusation. API calls such as canvas, WebGL, audio, or WebRTC access can be legitimate. Behavioral heuristics currently cover canvas readback after drawing, repeated canvas font measurement, WebGL entropy reads, offline audio rendering, WebRTC peer-connection setup, and third-party listener coverage; comparison runs are still required before making stronger claims. When the observer reads a frame but cannot attribute every event listener to the script that registered it (a call stack deeper than its bound, or more script origins than it tracks), it keeps that frame's canvas, WebGL, audio and WebRTC evidence and event counts, withholds the frame's session-recording and input-monitoring findings, and marks the fingerprinting evidence incomplete. A session-recording or input-monitoring finding that names a script origin with no publishable registrable domain (a path-style S3 host, a bare IP address, or a host that is itself a public suffix) is withheld whole, publishable origins included; the report says so (a fixed warning on v1, a recorded capture loss on r2) and treats listener findings as incomplete, though v1 reports sanitized before this warning existed carry none. The canvas heuristics read page canvases and OffscreenCanvas 2D contexts in the page alike: text drawing, pixel readback and font measurement on an OffscreenCanvas feed the same heuristics with the same thresholds, and an OffscreenCanvas export through convertToBlob is recorded, once its promise fulfills, as its own API, canvas.convertToBlob, never as canvas.toBlob, so an export still pending when the visit's evidence is collected is not recorded. A page canvas whose control was moved to an OffscreenCanvas with transferControlToOffscreen is read as showing that OffscreenCanvas's text. Text drawn on one canvas is traced into another canvas's readback only through drawImage, including drawImage of a bitmap made by transferToImageBitmap or createImageBitmap; any other route, such as an ImageBitmap handed to a bitmaprenderer context or a pattern made with createPattern, is not traced, for either kind of canvas. The observer hands the page its own promise for each promise call it records (convertToBlob, createImageBitmap, OfflineAudioContext startRendering, RTCPeerConnection createOffer and setLocalDescription), settling as the native one does, one microtask later, so a rejection the page leaves unhandled still reaches the page's unhandledrejection listeners. WebGL is observed on both kinds of canvas in the page. Canvas, font and WebGL use inside each dedicated worker the page starts is recorded by the same observer, installed in that worker before its first statement, and read back into the report at the same two moments as the page's own evidence, from workers whose document is still the page's current one: a worker of a document the page navigated away from is not credited. The heuristics apply to the page and to each worker separately, so text drawn in one and read back in another (drawn in a worker and read from an image the worker handed to the page, or drawn by a worker onto a canvas the page transferred to it and then exported by the page) counts as separate calls in each and does not by itself match a canvas heuristic, and font measurements split across them are counted the same way. A dedicated worker whose evidence cannot be read in full, for example one still in the middle of a task when the evidence is collected (a worker the page has just terminated included), one whose evidence has not all reached the scanner by then, or one the browser reported but the scanner never reached, is never read at an earlier state instead: it marks the visit's fingerprinting evidence incomplete, with a fixed warning and a recorded capture loss, and never lets the visit read clean. Shared workers and worklets are not instrumented. A shared worker the page starts marks the visit's fingerprinting evidence incomplete in the same way, although what it did is not observed; on the rare visit where the scanner's DevTools connection is unavailable, the scanner cannot see that a shared worker started at all, and a dedicated worker started inside a shared worker is seen by neither record (the shared worker's own loss covers the visit). In a consent comparison, an r2 report places each fingerprint detection and call count before or after the click only when the read taken just before the click covered every frame and every worker realm; a shared worker the page started by then, like a frame or dedicated worker that could not be read at that moment, therefore withholds every r2 fingerprint detection and call count of that visit, the page's own included, while its v1 report, which has no phases, keeps them. A shared worker started after the click marks the evidence incomplete without withholding them. Older reports did not observe OffscreenCanvas 2D work, or canvas, font and WebGL use inside a worker, at all: a v1 report whose methodology token lacks `+fingerprint-surface-v2` (or that records no token), and an r2 report whose fingerprint detector is `fingerprint-observer@4` or earlier (its methodology then also lacks `+worker-fingerprint-v1`), can read quiet for a page that drew, measured or read back text only on an OffscreenCanvas, or fingerprinted only inside a worker, and its fingerprint event counts omit that work. Reports measured before 2026-07-20 may show a WebGL detection recorded under the earlier single-signal rule (a WebGL parameter read or a pixel readback, where the current rule requires both); report pages label it as that earlier rule and present it at info level, not as a heuristic match.
