...
> **Operational follow-ups.** The repository's health checks prove configured posture, not every external control. WAF-ceiling verification, container-log retention/query verification, a scoped synthetic R2 write/read/delete monitor, and a platform-compatible independent egress backstop remain operator work.
...
**CI-gated deployment.** Production (Cloudflare Pages and the scanner's Workers Builds) tracks the **`production`** branch, never `main`. CI fast-forwards `production` to the exact SHA it tested only after its Typecheck/Unit Tests/Build, Chromium Smoke Test, and Docker Runtime and Public R2 Smoke jobs pass; the Docker job is a required promotion dependency, not an optional local check. `.github/workflows/promote-production.yml` provides an idempotent fallback for ordinary push/user-dispatched runs. Both paths share the same serialized, fast-forward-only checks: they never force-push, skip out-of-order completions, and hard-fail on a tested SHA no longer reachable from `main`. CI promotes directly because workflow runs dispatched by repo-writing workflows with `GITHUB_TOKEN` do not reliably cascade into a third `workflow_run`. Set the repository variable `SITE_BEHAVIOR_LAB_PROMOTION_PAUSED=1` to hold production during an incident, then clear it and rerun CI to resume. Operator setup, once: point the Cloudflare Pages project's production branch and the scanner's Workers Builds branch at `production`, and disable non-production builds on both.
...
The featured-scan workflow also runs on a weekly schedule (Mondays 05:23 UTC) with its default inputs (Shields comparisons over the featured catalog), so the corpus refreshes itself. A batch below the 90% health gate stays red and keeps its canonical repair issue open, while independently validated successful reports still publish; retention pruning is withheld so failed targets keep their older evidence. On `/directory/`, a site's newest report shows what changed only when a previous successful, uncapped report has the same kind and requested/final subject and a compatible schema revision, methodology, browser environment, device/viewport, intervention state, filter snapshot, and tracker catalog. Unknown or generalized v1 subjects fail closed. These deltas are observed differences between two automated visits, which can also reflect ad rotation, experiments, caching, or bot detection, and the UI says so.
