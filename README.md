...
The featured-scan workflow also runs on a weekly schedule (Mondays 05:23 UTC) with its default inputs (Shields comparisons over the featured catalog), so the corpus refreshes itself. On `/directory/`, each site's newest report shows what changed since the previous scan of the same kind ("+12 third-party, -3 tracker requests"); kinds never mix, because a consent comparison's lead run (post-accept) is not comparable to a Shields comparison's lead run (pre-consent). These deltas are observed differences between two automated visits, which can also reflect ad rotation, experiments, caching, or bot detection, and the UI says so.
