...
/**
 * Exact producer epoch of node-detectors-v6 under the ORIGINAL GPC worker
 * mechanism (deployed 2026-08-17 until gpc-worker-application-v2 revised how
 * the GPC arm treats Web Workers). The committed corpus holds no v6 report,
 * but live scans could publish under it during that window, so its rows stay
 * replayable. Same registry and detector versions as the active epoch: no
 * detector's semantics moved, so ONLY the methodology component separates
 * these closed rows from the active pair, and the methodology literal here
 * must never gain the `gpc-worker-application-v2` suffix. Every value is a
 * pinned literal per this file's closed-epoch rule.
 */
export const HISTORICAL_DETECTOR_V6_NODE_R2_METHODOLOGY_VERSION =
  "shields-request-context-v2-adblock-rust-0.13.2-request-method-v1-playwright-1.62.1+subject-validity-v2+detector-coverage-v2+phase-kernel-v2+boundary-state-v1+consent-r2-v4+resource-budget-v2+proxy-traffic-v1+service-worker-block-v1+detector-accountability-v1+service-role-taxonomy-v1";
export const HISTORICAL_DETECTOR_V6_NODE_R2_DETECTOR_REGISTRY_VERSION = "node-detectors-v6";
export const HISTORICAL_DETECTOR_V6_NODE_R2_DETECTOR_REGISTRY_DIGEST =
  "81866718b36e35239f0418cc543eee845660e686849da1b816d937a601c1528b";
export const HISTORICAL_DETECTOR_V6_NODE_R2_DETECTOR_VERSIONS = Object.freeze({
...
// Exact normalization of the node-detectors-v9 production producer, closed by
// the 2026-09 toolchain epoch when the public-suffix engine moved from
// tldts@7.4.10 to tldts@7.4.13. That producer kept the v10 methodology above:
...
// active-probe-v3 move the methodology, and four admitted warnings (the v1
// listener-withheld disclosure and the input probe's unread-request,
// incomplete-test and stopped-navigation lines) move the public-string policy
// digest.
...
> = Object.freeze({
  ...
  // lines. Only the 2026-09 toolchain epoch's rows ran it, all under its one
  // methodology.
  ...
  // methodology.
  [HISTORICAL_NODE_V11_NORMALIZATION]: Object.freeze([HISTORICAL_NODE_V10_METHODOLOGY]),
...
/**
 * Frozen copy of the September 21 list snapshot, as the node-detectors-v9
 * production rows published it under adblock-rust 0.13.2. The live constant
 * above derives its engine from NODE_ADBLOCK_ENGINE_VERSION, which the
 * 2026-09 toolchain epoch moved to adblock-rust-0.13.3; this closed copy
 * keeps the engine those reports were measured with.
 */
export const HISTORICAL_R2_LISTS_2026_09_21_ADBLOCK_IDENTITY = Object.freeze({
  source: "Brave default ad-block lists",
  lists: 31,
  fetchedAt: "2026-09-21T12:45:16.395Z",
  manifestDigest: "7e42412ee50d641b83699aff20bb2d2151a36b5419121f7245ab0dec5b243a9f",
...
const HISTORICAL_DETECTOR_V6_REGISTRY = Object.freeze({
  version: HISTORICAL_DETECTOR_V6_NODE_R2_DETECTOR_REGISTRY_VERSION,
  digest: HISTORICAL_DETECTOR_V6_NODE_R2_DETECTOR_REGISTRY_DIGEST
});
const HISTORICAL_DETECTOR_V6_FIELDS: NodeTupleFields = Object.freeze({
  detectorRegistry: HISTORICAL_DETECTOR_V6_REGISTRY,
  detectorVersions: HISTORICAL_DETECTOR_V6_NODE_R2_DETECTOR_VERSIONS,
  detectorStatusContractVersion: "detector-status-v2",
  detectorObligations: HISTORICAL_SERVICE_ROLE_V1_NODE_R2_DETECTOR_OBLIGATIONS,
  serviceRoleTaxonomy: HISTORICAL_SERVICE_ROLE_V1_NODE_R2_SERVICE_ROLE_TAXONOMY,
  trackerCatalog: HISTORICAL_R2_2026_08_TRACKER_CATALOG,
...
const HISTORICAL_NODE_V9_FIELDS: NodeTupleFields = Object.freeze({
  detectorRegistry: Object.freeze({
    ...
    "digest": "b15c8281f0db49b91a46427ffee63e44bf7bbbfc0a9878069c2bb1098b6d4715"
  } as const),
  detectorVersions: Object.freeze({
...
export const NODE_R2_PRODUCER_TUPLES: readonly NodeR2ProducerTuple[] = Object.freeze([
  ...
  // Closed: node-detectors-v6 under the original GPC worker mechanism was the
  // deployed producer from 2026-08-17 until gpc-worker-application-v2. The
  // committed corpus holds no v6 report, but live scans could publish during
  // that window, so the rows stay replayable; only the methodology component
  // separates them from the active pair.
  nodeTuple(
  ...
  nodeTuple("node-v8-pixel-coverage-active-lists-2026-08-15", HISTORICAL_NODE_V7_NORMALIZATION,
  ...
  // The node-detectors-v9 production producer, with and without the September
  // 21 lists, closed to its exact source identity by the 2026-09 toolchain
  // epoch: Playwright 1.62.1, adblock-rust 0.13.2 and tldts 7.4.10.
  nodeTuple("node-v11-detectors-v9-active-lists-2026-09-21",
    HISTORICAL_NODE_V11_NORMALIZATION, HISTORICAL_NODE_V10_METHODOLOGY,
    HISTORICAL_NODE_V9_FIELDS, HISTORICAL_R2_LISTS_2026_09_21_ADBLOCK_IDENTITY
  ),
  nodeTuple("node-v11-detectors-v9-active-no-adblock",
    HISTORICAL_NODE_V11_NORMALIZATION, HISTORICAL_NODE_V10_METHODOLOGY,
...
export const PAGEGRAPH_R2_PRODUCER_TUPLES: readonly PageGraphR2ProducerTuple[] = Object.freeze([
  ...
  pageGraphTuple("pagegraph-v4-active",
  ...
  // Closed by the 2026-09 toolchain epoch, which moved the public-suffix engine
  // from tldts@7.4.10 to tldts@7.4.13 under the same policy digest.
  pageGraphTuple("pagegraph-v4-listener-warning-active",
  ...
  // Closed by the four warnings the node-detectors-v10 measurement epoch
  // admitted (the v1 listener-withheld disclosure and the input probe's
  // unread-request, incomplete-test and stopped-navigation lines); the
  // public-string policy digest is shared by both observers.
