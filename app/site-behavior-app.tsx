...
type Level = "ok" | "quiet" | "info" | "warn" | "loud";
...
function levelForMetric(metric: BenchmarkMetric, value: number): Level {
...
function strongestLevel(levels: Level[]): Level {
  const order: Level[] = ["ok", "quiet", "info", "warn", "loud"];
...
function fingerprintDetections(result: ScanResult): FingerprintDetectionSummary[] {
  return result.fingerprintDetections ?? [];
}

function fingerprintDetection<K extends FingerprintDetectionSummary["kind"]>(
  result: ScanResult,
  kind: K
...
function fingerprintDetectionCount(result: ScanResult): number {
  return fingerprintDetections(result).reduce((total, detection) => total + detection.count, 0);
}

function humanList(items: string[], limit = 3): string {
...
function buildFindings(report: ScanReport, result: ScanResult, corpus: CorpusStats | null): Finding[] {
  ...
  if (isComparisonReport(report) && report.comparisonType === "shields") {
    ...
    findings.unshift({
      ...
      lead:
        blockedTotal > 0
          ? `Shields on showed ${removedThirdPartyRequests.toLocaleString()} fewer third-party and ${removedTrackerRequests.toLocaleString()} fewer known-service requests in this single paired visit.`
      ...
      evidence: `${removedCookies.toLocaleString()} fewer third-party cookies and ${removedFingerprintEvents.toLocaleString()} fewer fingerprint-like calls with Shields on.`
    });
  }
  ...
  findings.unshift({
    id: "bottom-line",
    icon: overallLevel === "ok" ? CheckCircle2 : AlertTriangle,
    level: overallLevel,
  ...
  if (result.conditions.adblock?.active) {
    ...
    findings.splice(1, 0, {
      ...
      title:
        blocked > 0
          ? `Brave Shields would block ${blocked.toLocaleString()} of ${result.summary.totalRequests.toLocaleString()} requests`
...
function requestProvenanceSummary(request: NetworkRequestRecord): { primary: string; secondary?: string } | null {
  const provenance = request.provenance;
...
function requestProvenanceSearchText(request: NetworkRequestRecord): string {
  const summary = requestProvenanceSummary(request);
  return summary ? `${summary.primary} ${summary.secondary ?? ""}` : "";
}

function provenanceChangeText(change: ProvenanceChange): string {
  const parts = [
...
function detectionLabel(detection: FingerprintDetectionSummary): string {
...
function detectionEvidence(detection: FingerprintDetectionSummary): string {
