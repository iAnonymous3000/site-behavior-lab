...
The review found that pasted URLs were normalized for the matching count and
profile shortcut but passed raw to the table, leaving it empty. Both now use
the normalized query. The narrow search layout also resets the input's flex
basis when the row becomes a column, so its desktop width does not become a
260 px height.

The static browser smoke test still expected the removed category select.
It now checks explicit category links, URL search/count/shortcut agreement,
empty and cleared searches, and narrow-screen input size. Report checks also
require findings without JavaScript and agreement between the server summary
and the explorer reached through an evidence link. Existing accessibility,
payload budgets, export, correction, and deployment gates remain in place.
