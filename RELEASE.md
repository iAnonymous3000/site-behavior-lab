...
The current release is `v0.6.0`, declared on 2026-09-06 and tagged on
2026-09-24 at `ee525b5fabc5ba4cc23dad4b9a7ea53f595d6217` by Cut Release Tag run
[`36047081291`](https://github.com/iAnonymous3000/site-behavior-lab/actions/runs/36047081291),
with its receipt durably archived at
[`docs/release-receipts/0.6.0/release-receipt.json`](docs/release-receipts/0.6.0/release-receipt.json).
The earlier `0.5.0` declaration was never tagged; its dated milestone remains in
the changelog. `CITATION.cff` cites `0.6.0`, and the policy no longer carries
`tagPending`.

The governed tag authority is configured: the dedicated release App
`sbl-release-tagger-ianon` (Integration `5055694`), its private key only on the
`release-tag` environment, and the creation-only tag ruleset `23918431`. The
0.6.0 ceremony was selected by governance receipt
`research/ops-receipts/release-tag-governance/a878e7bfdb7626ec6d54c8dfbf7bbee746970210eea0f094ea1517af0b0ab913.json`.
A later release captures its own fresh receipt under step 3; never create a
tag through a manual push or weaken the existing tag protections.
...
4. Run the **Cut Release Tag** workflow with that version **and the exact
   ...
   **only** as the `RELEASE_APP_PRIVATE_KEY` secret on the `release-tag`
   ...
   For a `0.x` release that report still scores the 1.0 gates, so its
   release-tag-governance gate reports the missing measurement-candidate
   binding. That is expected: Cut Release Tag runs the readiness check only for
   `1.0.0` and `1.0.0-rc.N`, and a `0.x` dispatch carries the `not-required`
   binding sentinel instead. For `0.x`, the selection check below is the gate.
   ...
   static build refuses when `GITHUB_SHA` (the dispatch branch tip) differs
   from the checked-out revision, and a workflow cannot override `GITHUB_SHA`.
   Push nothing to `main` between the carrier commit and the dispatch; if
   `main` moves, dispatch its new tip once that tip contains the receipt and
...
The archive PR must also advance `CITATION.cff` to the released version and
the receipt's release date, and remove `tagPending` from `release-policy.json`,
in the same tree as the receipt. Those two edits close the declare-then-tag
window, and guards enforce both directions: `CITATION.cff` may not cite the
...
build commit, the independently executed Node 24.20.0 version, and the asserted
...
The digest-pinned Playwright base is verified at Node 24.20.0 with npm 11.19.0
