...
| `SITE_BEHAVIOR_LAB_CONSENT_VERIFICATION` | unset | Node scanner only. Set to `1` to read back the site's registered consent state after a dispatched banner click (TCF `__tcfapi` and OneTrust consent-cookie interpreters, plus banner-visibility observations) and to attempt one disclosed post-choice page reload that re-reads the registered state; the reload's requests are excluded from the recorded request log and counts. The readback is recorded for the staged v2 report schema; v1 report content is unchanged apart from the reload disclosure warning. |
| `SITE_BEHAVIOR_LAB_V2_SHADOW_EMISSION` | unset | Node scanner only. Set to `1` to additionally build ScanReport v2/r2 public wires (redacted, screenshots stripped) in `SITE_BEHAVIOR_LAB_V2_SHADOW_DIR` (default `.site-behavior-lab/v2-shadow`): one single artifact for a single scan, or one complete pair artifact for a comparison. Comparison axis, semantic arms, execution order, verification, comparability, and diff are derived from the two visits' staged facts; partial per-arm files are never written. Shadow output is operator-local: producers keep emitting v1 publicly, and a failed shadow build is a logged diagnostic, never a failed scan. Requires `SITE_BEHAVIOR_LAB_BUILD_COMMIT`; observe-mode visits also need the consent-verification flag so the always-on banner detector records a real outcome. |
| `SITE_BEHAVIOR_LAB_ASYNC_SCANS` | unset | Set to `1` to make `/api/scan` return `202 { jobId, statusPath, reportId }` and run scans through the single-process in-memory job queue. Clients poll `/api/scans/:jobId` until the report is ready, and `DELETE /api/scans/:jobId` cancels a queued or running job cooperatively (a job whose report save has started can no longer be cancelled). Completed async reports are saved under the separate `reportId` from the submission response (never the job ID, so a shared report link cannot reach the status channel). The Containers front Worker keeps TTL-bounded job/report capability metadata in its existing Durable Object: after a container restart it can recover a persisted report or name the job as unrecoverable, without storing the target URL or client identifier. Queued/running execution itself remains in-memory. |
...
Horizontally scaled deployments should replace or front the filesystem report store with shared durable storage and external queueing. The Cloudflare Containers front worker accounts public quotas atomically in its singleton Durable Object, while the Node process retains a second in-process defense. Its bounded IDs-only job registry makes restart recovery explicit and recovers completed reports that reached R2, but queued/running execution is not replayed after a process restart.
