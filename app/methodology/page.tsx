  ...
  return (
    <SiteChrome activePath="/methodology/">
      ...
      <section className="legal-section">
        ...
        <p>
          A visit that requests Global Privacy Control blocks one more class of worker, and only that visit does.
          Putting the signal inside a worker means instrumenting the worker&apos;s script before its first
          statement, which cannot be done for a <code>blob:</code> or <code>data:</code> script URL without
          changing that worker&apos;s URL, origin, and content-security context. Rather than change what it is
          measuring, the scanner refuses it: the page&apos;s <code>Worker</code> constructor throws and that worker
          never runs. A visit where this happened says so in its warnings and marks its request evidence
          incomplete. It also means a GPC comparison differs between its two visits in more than the GPC signal
          itself, on sites that build workers from <code>blob:</code> URLs.
        </p>
      </section>
