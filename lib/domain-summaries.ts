...
/**
 * Pure per-domain grouping of a request log (no public-suffix list: the
 * thirdParty/tracker flags were classified at capture time and ride on each
 * record). Split from domain-utils so the runtime-light view seam
 * (lib/scan-report-views.ts) can derive a v2 run's domain table without
 * pulling tldts into the client bundle.
 */
