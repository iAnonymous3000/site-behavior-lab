...
| `SITE_BEHAVIOR_LAB_V2_SHADOW_EMISSION` | unset | Node scanner only. Set to `1` to additionally build a ScanReport v2/r2 from each completed visit's staged phase-aware facts and write the public wire (redacted, screenshot stripped) to `SITE_BEHAVIOR_LAB_V2_SHADOW_DIR` (default `.site-behavior-lab/v2-shadow`). Shadow output is operator-local: producers keep emitting v1 publicly, and a failed shadow build is a logged diagnostic, never a failed scan. Requires `SITE_BEHAVIOR_LAB_BUILD_COMMIT`; observe-mode visits also need the consent-verification flag so the always-on banner detector records a real outcome. |
