...
function isScannerWarning(warning: string): boolean {
  ...
  if (/^This scan typed a synthetic test value into (?:1 form field|[0-9]+ form fields) with native form submission blocked\. Focus, input and blur handlers may run and send requests\. The value is synthetic and is not stored\. (?:Observed requests during typing and the following wait are included in the request log\. Teardown-only transmissions are not measured\.|Requests from this incomplete probe were omitted from the recorded request log and counts\.)$/.test(warning)) {
    return true;
  }
  // Historical disclosures remain readable; correction context qualifies them.
