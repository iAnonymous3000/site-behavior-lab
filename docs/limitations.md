...
Saved reports pass through the sanitizer described in `docs/scan-report-v2-rfc.md` section 9; its public-string rules are named in each r2 report's normalization identity (currently `public-string-policy-v4`). A hosting tenant label, the first label of a registrable domain under a shared provider suffix such as `akamaihd.net` or `netlify.app`, is generalized to `{label}` when it holds a dashed or underscored IPv4 address, a run of eight or more digits, a hex token of twelve or more characters mixing digits and letters, or a token of sixteen or more characters with at least three separate digit runs. The RFC's section 9.1 describes generalization only for subdomain labels left of the registrable domain; r2 reports also count each generalized tenant label in `privacy.redaction.subdomainLabelsGeneralized`, so that count is not limited to labels left of the registrable domain. The shapes close the per-visit patterns seen in published reports, not every identifier a provider can mint: a dashed IPv6 address, a short token, or a token with fewer than three digit runs survives, and a version-like label such as `v1-2-3-4` is generalized too. A site whose own host carries such a tenant label cannot be scanned, because its report could not keep a stable subject. A saved r2 report holding a string a later rule removes is no longer served and is not rewritten; the public instance stops serving saved reports at seven days and its bucket deletes them at eight. A committed report holding one is replaced for privacy under a new report ID (see `docs/corrections-ledger.md`); the replacement reduces what is served, and git history and earlier release archives still hold the original bytes.
