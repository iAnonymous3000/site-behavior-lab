...
const OUTPUT_LIMIT = 48 * 1024 * 1024;
const PAGE_LIMIT = 10;
...
async function collect(options) {
  ...
  const request = async (
    ...
    return githubApi(endpoint, token, maximumBytes, accept);
  };
  requireValue(
  ...
  for (const { listed, runId, currentAttempt } of deepRuns) {
    ...
    for (const attempt of deepAttempts) {
      ...
      members.set(
        ...
        downloaded.members.get("production-health.json")
      );
    }
  }
  ...
  requireValue(
    totalBytes > 0 && totalBytes <= OUTPUT_LIMIT,
    `durable soak aggregate exceeds ${OUTPUT_LIMIT} retained bytes`
  );
