...
`
  ...
  const censoringPolicyPath = MEASUREMENT_CALIBRATION_POLICY_ASSIGNMENTS_PATH;
  ...
  // Producer-generated by provenance: the fixture copies the REPOSITORY's
  // committed assignments artifact (whose derivation the producer --check
  // pins in CI) rather than restating any field of it. The invalid leg
  // tampers a semantic field and re-serializes canonically so only the
  // assignments verifier can refuse it.
  {
    const committedPolicy = JSON.parse(
      readFileSync(
        path.join(process.cwd(), ...MEASUREMENT_CALIBRATION_POLICY_ASSIGNMENTS_PATH.split("/")),
        "utf8"
      )
    ) as Record<string, unknown>;
    if (options.invalidCensoringPolicy) {
      (committedPolicy.censorReasons as string[])[3] = "not-an-analyzer-reason";
    }
    const policyDestination = path.join(root, ...censoringPolicyPath.split("/"));
    mkdirSync(path.dirname(policyDestination), { recursive: true });
    writeFileSync(policyDestination, `${JSON.stringify(committedPolicy, null, 2)}\n`);
  }
  const censoringPolicySha256 = sha256File(
    path.join(root, ...censoringPolicyPath.split("/"))
  );
  const fixturePolicyArtifact = JSON.parse(
    readFileSync(path.join(root, ...censoringPolicyPath.split("/")), "utf8")
  ) as Record<string, unknown>;
  const censoringDispositionSha256 =
    measurementCalibrationPolicyAssignmentsDispositionSha256({
      ...
      analyzerVersion: String(fixturePolicyArtifact.analyzerVersion),
      detectors: measurementCalibrationAssignmentsSemanticProjection(
        fixturePolicyArtifact as never
      )
    });
  ...
  return {
    ...
    calibrationPolicy: {
      ...
      policyArtifactSha256: sha256File(
        path.join(
          root,
          ...MEASUREMENT_CALIBRATION_POLICY_ASSIGNMENTS_PATH.split("/")
        )
      ),
      dispositionSha256: (
        JSON.parse(
