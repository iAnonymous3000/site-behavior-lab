...
The command fails closed on degraded or stale health, missing sandbox/R2/durable staging attestations, a failed, bot-walled, unsettled, budget-exhausted or not run-level complete scan, an evidence family censored by anything other than its own recorded capture loss, a capture-loss signature (family, kind and detail) that differs between any two runs of a site across both builds, subject or egress drift, mixed provenance inside a capture, incomplete panel coverage, or a median outside the committed absolute/relative tolerance. A capture loss that every run of a site records in both builds is compared like with like: the metrics its evidence family feeds are left out for that site and printed as `LEFT OUT`, as the report's claim eligibility (`REPORT_CLAIM_REQUIREMENTS`) treats a censored family, and the gate fails if that would leave a metric compared on no site. A loss recorded without a detail is identified by family and kind alone. Since node-detectors-v9 the fingerprint listener-attribution loss fires on most ad-heavy pages, so `fingerprintEvents` is usually left out for the heavy news case and compared only on the lighter sites; a keystroke-probe truncation censors detector output, which no canary metric is counted from. It submits ordinary token-gated single scans only; it never sends the staging fault-injection headers. Receipts are mode `0600`, stay outside `public/`, contain no access token, and record the exact staging report IDs for audit and teardown.
