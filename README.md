...
**CI-gated deployment.** Production (Cloudflare Pages and the scanner's Workers Builds) tracks the **`production`** branch, never `main`. After all required CI jobs pass, CI fast-forwards `production` to the exact SHA it tested; `.github/workflows/promote-production.yml` provides an idempotent fallback for ordinary push/user-dispatched runs. Both paths share the same serialized, fast-forward-only checks: they never force-push, skip out-of-order completions, and hard-fail on a tested SHA no longer reachable from `main`. CI promotes directly because workflow runs dispatched by repo-writing workflows with `GITHUB_TOKEN` do not reliably cascade into a third `workflow_run`. Set the repository variable `SITE_BEHAVIOR_LAB_PROMOTION_PAUSED=1` to hold production during an incident, then clear it and rerun CI to resume. Operator setup, once: point the Cloudflare Pages project's production branch and the scanner's Workers Builds branch at `production`, and disable non-production builds on both.
...
`npm run corpus:stats` (`scripts/build-corpus-stats.mjs`) reads the committed reports under `public/reports/` and builds the current **legacy-v1 percentile cohort**. It keeps the newest eligible lead run per distinct non-reserved site: the main document must have loaded successfully, request evidence must be uncapped, and the run must remain in passive `observe` consent state. Failed/no-response, request-capped, accept/reject consent, and every v2 run are excluded from the distribution. Successful v2 and capped runs still contribute to `coverageSiteCount`, so coverage and statistical measurement remain separate. The Pages build and both scan workflows rebuild the artifact after the report manifest.
...
Rows with a null status or status `>= 400`, request-capped rows, and post-choice consent lead runs remain visible as evidence but stay out of this project's percentiles and aggregate cohorts. The export is deliberately mixed-version: historical v1 rows are `legacy-derived` and `limited`, while r2 rows expose their native revision and claim strength. Current percentiles remain v1-only; category rollups use their separately disclosed eligible cross-version cohort. Filter by `schema_version`, `schema_revision`, `schema_origin`, and `limited` before aggregation.
...
CI also runs the dependency audit, static export, and static smoke test; the separate smoke job runs a built Node app in Chromium.
