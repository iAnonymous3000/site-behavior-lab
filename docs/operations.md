...
For the exact flag-off and Phase-2 contracts, see [docs/scan-job-model.md](scan-job-model.md).
...
The reference deployment does not build on Cloudflare. Main CI runs `npm run build:pages` with the production site URL, scan API origin, and Turnstile site key compiled in, and after promotion `Deploy Tested Pages` (`.github/workflows/deploy-pages.yml`) verifies that attested `out/` export against its signed byte manifest and uploads it to the existing Pages project with Wrangler (the cutover and restore steps are in `docs/deploy-cloudflare-containers.md`). Those public settings therefore belong in the CI build environment: the Pages dashboard cannot change constants already compiled into the export. A self-hosted Pages project that builds from Git instead sets the production branch to `production`, the build command to `npm run build:pages` and the output directory to `out`, with production env vars `SITE_BEHAVIOR_LAB_PAGES_BASE_PATH=/`, `NEXT_PUBLIC_SITE_BEHAVIOR_LAB_SITE_URL` set to its public origin, and `NEXT_PUBLIC_SITE_BEHAVIOR_LAB_SCAN_API_BASE` pointed at its scan API; never run such a build beside the attested upload, so that one deployment writer remains. `production` is the CI-gated promotion branch (see "CI-gated deployment" above): only the exact `main` SHA a successful CI run tested ever reaches it, so a red revision cannot publish. Any other static host can serve the same `out/` directory.
...
after each job. See [docs/featured-corpus-r2-rollout.md](featured-corpus-r2-rollout.md).
...
> **Preview a corpus run before publishing.** The workflow pushes the new reports to a per-attempt `automation/*` branch and opens a pull request against the branch it runs on, so the corpus never lands anywhere without review. Dispatch it from `main` or a staging branch (in **Actions > Scan Featured Sites**, choose the branch under "Use workflow from"); the proposal PR is the staging surface. Pages automatic preview deployments are disabled in the current deployment (existing preview deployments remain Access-protected), so a staged branch gets no preview; review it locally with `NEXT_PUBLIC_SITE_BEHAVIOR_LAB_SITE_URL=https://example.org npm run build:pages` on a clean checkout of the branch, check the gallery, `/directory/`, and whether the corpus cleared `CORPUS_MIN_SAMPLE` (the run's job summary reports the report and distinct-site counts), then merge the proposal if its base is `main`; the trusted CI, attestation, and promotion chain runs only on `main`, so a proposal merged into any other branch publishes nothing.
...
[docs/deploy-cloudflare-containers.md](deploy-cloudflare-containers.md) for
Cloudflare Containers, or [docs/deploy-node-container.md](deploy-node-container.md)
...
archived durably under [docs/release-receipts/](release-receipts/). The
