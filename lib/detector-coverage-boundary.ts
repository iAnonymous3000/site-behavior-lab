...
export const COVERAGE_BOUNDARY_ENTRIES: readonly CoverageBoundaryEntry[] = [
  ...
  {
    id: "cross-realm-canvas",
    label: "Canvas work split across realms, and shared workers and worklets",
    reason: "not-instrumented",
    explanation:
      "A script can move canvas work into a worker: it can create an OffscreenCanvas there, or transfer one from the page, and draw text, measure fonts, read pixels, export an image, or read WebGL parameters inside the worker. The scanner observes that work inside every dedicated worker the page starts, with the page's own observer and heuristics, and credits a worker only while its document is still the page's current one, but it applies its canvas and font heuristics to the page and to each worker separately. Text drawn in one realm and read back in another, for example drawn in a worker and read from an image the worker handed to the page, or drawn by a worker onto a canvas the page transferred to it and then exported by the page, is counted as separate calls in each realm and does not by itself match a canvas heuristic; font measurements split across realms are counted the same way. A dedicated worker whose evidence cannot be read in full, such as one still in the middle of a task when the visit's evidence is collected, marks the visit's fingerprinting evidence incomplete rather than clean. Shared workers and worklets are not instrumented: when the scanner sees that a page started a shared worker it marks the visit's fingerprinting evidence incomplete rather than clean, but it does not observe what the shared worker did, and on the rare visit where its DevTools connection is unavailable it cannot see that a shared worker started at all. Service workers are blocked and never run. In the page itself, 2D work on an OffscreenCanvas feeds the same canvas heuristics as work on a page canvas. Text drawn on one canvas is traced into another canvas's readback only through drawImage (including drawImage of a bitmap made by transferToImageBitmap or createImageBitmap) and into the readback of a canvas whose control was moved with transferControlToOffscreen; any other route, such as an ImageBitmap handed to a bitmaprenderer context or a pattern made with createPattern, is not traced, for either kind of canvas. An OffscreenCanvas export still pending when the visit's evidence is collected is not recorded. Reports measured before node-detectors-v11 (an r2 report whose fingerprint detector is fingerprint-observer@4 or earlier, its methodology then also lacking +worker-fingerprint-v1, or a v1 report whose methodology lacks +fingerprint-surface-v2) observed neither OffscreenCanvas 2D work in the page nor any canvas, font or WebGL work inside a worker, so a quiet canvas, font or WebGL finding on such a report does not rule that work out."
  },
  {
