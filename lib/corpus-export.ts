...
export const CORPUS_EXPORT_NOTE =
  "One row per published report. A single report records one automated, controlled Chromium visit; a comparison report pairs two such visits, one per compared condition. The corpus is a curated set of sites (popular, mostly commercial, plus a diversity seed list), not a random sample of the web, so treat cross-site statistics as describing this corpus only. Counts use the report's lead run (the unprotected baseline on Shields/GPC comparisons, the accept-all run on consent comparisons) and are lower bounds. On consent rows, consent_clicks records which banner choices the scanner dispatched a click for; it is a dispatch column, not a verification column. V1 never recorded whether the site registered a choice, while r2 reports carry recorded verification observations and default-deny claim gates in the linked evidence. Every run's totals include pre-click traffic; rows without accept-and-reject have at least one run still in the pre-consent state (the scanner found no clickable control for that choice), so their diffs do not compare the two choices; an accept-only or reject-only row still mixes one visit where a click was dispatched (its recording spans pre- and post-click traffic) with one pre-consent visit. Rows with a status of 400 or higher reflect an error or block page (the site refusing the automated visit), not the site's normal behavior, and rows with request_capped true hit the scanner's 1,000-request recording cap, so their activity counts are floors cut off mid-collection and their cookie and storage figures are end-state snapshots of an interrupted visit. Those rows, plus post-choice consent lead runs, stay out of this project's percentiles, category medians, leaderboards, and since-last-scan deltas. siteCount counts distinct sites with at least one successful load, including request-capped recordings. measuredSampleSize is the exact current percentile cohort: distinct sites whose newest eligible legacy-v1 lead run loaded successfully, was not capped, and remained in the passive observe consent state. Category medians and leaderboards apply the same passive-run exclusions across every supported schema generation, so their cross-version cohort can differ from measuredSampleSize. Delta fields compare a site's newest report against its previous successfully loaded, uncapped report only when kind, requested/final subject, schema revision, methodology, browser environment, device/viewport, intervention state, filter snapshot, and tracker-catalog identity are compatible; an unknown setup never matches another unknown. The deltas can still reflect run-to-run variance as well as real site changes. schema_version/schema_revision record the report's wire generation, schema_origin marks legacy-derived (v1) rows whose derived facts were never recorded as v2 fact, and limited marks rows whose schema revision supports only descriptive (never causal) claims. Historical v1 rows are legacy-derived and limited; r2 rows carry recorded facts and may be non-limited. Filter by these columns before aggregation: this project's current percentile distributions remain v1-only and never mix r2 metrics into the legacy cohort. shields_third_party_change is the SIGNED third-party request change of an eligible Brave-list blocking pair: the blocking visit's count minus the unblocked baseline's (Brave's ad-block engine and default Shields lists, simulated in this scanner's browser, not a live Brave-browser visit), so a negative value means fewer requests with blocking on and a positive value means more; increases are real paired-visit observations (ad rotation, fallback loading) and are reported signed, not clamped to zero. It is never a count of individually blocked requests, which each blocking run records separately. shieldsChangeSummary in this payload counts the rows with a non-null change by direction (decreased / flat / increased). Full methodology and per-report evidence are linked from each row.";
