...
2. Push the validated deployment change to main and require CI, registry publication,
...
3. Disable the scanner's automatic Workers Builds production deployments and
   ensure no old build remains queued or running. The separate Pages handoff
   below controls its deployment writer; both keep `production` as their branch.
...
## Production Pages deployment from the tested export

`Deploy Tested Pages` uses the existing signed static release manifest. It
resolves the successful main run for the exact promoted SHA, verifies its
attestation, and compares every downloaded file with the signed byte manifest.
Only then does Wrangler upload the static export to the existing Pages project.
There is no build step, artifact substitution, or fallback to unsigned files.
Provider success, the live deployment receipt, corpus index, schema, and a
compiled browser asset must agree before the independent health run is requested.

CI builds the production root URL, scanner origin, and public Turnstile site
key. Changes to those public settings belong in the CI build environment; the
Cloudflare dashboard cannot change constants already compiled into an export.
The static browser checks use an explicit test health response and inert
Turnstile script. They exercise UI contracts without claiming a live scan or
CAPTCHA verification. Independent production health exercises the live service.

For cutover, add Account / Cloudflare Pages / Edit to the dedicated deployment
token, restricted to the existing account. Disable automatic Pages production
deployments and keep previews disabled, then set repository variable
`SITE_BEHAVIOR_LAB_PREBUILT_PAGES_DEPLOY=1`. Do this before the first production
promotion with the new workflow. The project, domain and production branch
remain unchanged. Require both workflow readback and deep Production Health.

The transfer artifact expires after one day; signed manifests and rollout
receipts remain for 90 days. Within that window, retry the same production
workflow. If the artifact has expired, rerun trusted main CI on the exact
production SHA to regenerate and attest it, then retry deployment. Never rebuild
inside the deploy job or claim that newly generated bytes were previously tested.
To restore native builds, first disable the prebuilt Pages workflow variable,
then re-enable automatic production builds, preserving one deployment writer.
No paid runner upgrade, new project, or extra container capacity is required.
