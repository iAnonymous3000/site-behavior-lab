...
Each visit is otherwise passive (no scrolling or clicking) except for the consent-banner click and active input probe. The probe types a synthetic, non-personal sentinel into supported fields already in the viewport, blocks native form submission and every navigation started while it runs, whether or not the probe caused it, and watches retained requests for that value leaving the site. A blocked navigation never loads: it is left out of the request log and names no recipient, and the visit's request evidence reads as incomplete because of it (r2 reports record it as lost request coverage, v1 reports with a fixed warning). One that would have carried the value to a third party, a request whose URL or body the probe could not read in full (including one past its capture bounds), a field it could not test or that refused the value, a probe that had no time to start or whose own work failed, and a page that left the recorded site before or during the probe each leave the input check incomplete rather than reporting that the value stayed. v1 reports, which have no detector status, say so with a fixed warning (for a page that left the site, the warning that already said so), though v1 reports measured before the newer warnings existed carry none. Focus, input and blur handlers can run and send requests. Offscreen fields, constrained number/date fields and failed attempts count as omitted coverage, not site prevention. Auxiliary pages are blocked; r2 reports record their omitted request coverage, and v1 reports carry no record of it. The scanner does not navigate away to flush recorders: teardown-only transmissions are not measured. These observations are lower bounds and do not cover login flows, later steps, untested fields or other frames.
