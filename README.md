...
- Evidence report with:
  ...
  - consent comparison (Node scanner): two paired visits, one clicking the banner's "Accept all" and one clicking "Reject all" (known CMP controls first, then a conservative whole-label text match; first banner layer only), diffed to show what the visitor's choice actually changed and which tracking companies survived a real rejection. When no control can be clicked, the run is disclosed as pre-consent and no claim is made
  ...
  - privacy-policy cross-check (Node scanner): the scanner discovers the site's privacy-policy link, reads the policy through the same SSRF-guarded browser context, and compares its text against the visit's evidence. It flags contradictions of checkable statements (a "we do not use third-party cookies" claim against observed third-party cookies, a "we do not sell personal information" claim against advertising pixels that carried hashed personal identifiers, an "we honor Global Privacy Control" claim against a GPC comparison that showed no real change) and lists observed tracking companies the policy never names. Every match quotes the policy sentence so it can be verified in context; it is an automated text match, not a legal reading
  - screenshot
