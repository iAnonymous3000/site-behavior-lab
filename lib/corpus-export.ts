...
export const CORPUS_EXPORT_NOTE = [
  ...
  "Rows with request_evidence_complete false have lower-bound request counts and stay out of this project's percentiles, category medians, and since-last-scan deltas, as do failed loads and post-choice consent lead runs. A request-capped row also has cookie and storage figures that are end-state snapshots of an interrupted visit.",
  ...
  "corpus_cohort_id is a versioned schema, methodology, tracker-catalog, read-time ServiceRole-taxonomy, metric-contract, recorded-producer, and requested-GPC identity. tracker_catalog_digest is the recorded content digest on v2 and a canonical hash of the available recorded legacy metadata on v1; tracker_catalog_origin keeps those assurance levels distinct. corpus_cohort_denominator is the distinct-site passive-run denominator for that exact cohort; no percentile or category median silently pools v1 and r2 or different methodology, catalog, ServiceRole, or metric-contract identities.",
